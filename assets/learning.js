(function () {
  ...
  function renderModule(module,step=-1,focus=true){
    ...
    if(state.step<0)content=`<section class="card"><p class="kicker">Mission 0${m.id} · ${m.duration} min</p><h1>${esc(m.title)} un logigramme</h1><p>${esc(m.intro)}</p><p>Prépare ton brouillon. Les tableaux se remplissent ligne par ligne. Pour ordonner un chemin, sélectionne les étapes puis utilise « Vérifier ».</p><p>Les réponses écrites sont de courtes justifications (${maxNote} caractères maximum). Elles seront relues par ton professeur. Tu peux aussi préparer ta phrase oralement avant de l’écrire.</p><div class="button-row"><button data-step="0" class="primary">Entrer dans la mission →</button><button data-speak-text="${esc(m.intro)}">Écouter le départ</button></div></section>`;
      ...
      <details class="extension"><summary>Pour aller plus loin · Sur ton brouillon</summary><p>${extension(m.id,state.step)}</p><p>Présente ton chemin au professeur ou à un camarade et teste un cas VRAI et un cas FAUX.</p></details>`;
    }
    app.innerHTML=path(m)+content;updateChrome();save();if(focus)focusHeading();
  }
  function extension(m,s){return m===1?'Dessine un système qui lit un bouton, allume une lampe pendant 2 secondes si le bouton est appuyé, éteint la lampe puis recommence. Ajoute le chemin quand le bouton n’est pas appuyé.':m===2?'Dessine la version du distributeur qui attend le retrait de la main avant une nouvelle dose. Où placer une condition supplémentaire ?':s===5?'Modifie le programme de la serre pour qu’elle attende 10 secondes avant chaque nouvelle mesure. Vérifie que la pompe est arrêtée pendant cette attente.':'Invente une seule erreur différente dans le programme de cette étape. Fais la diagnostiquer à un camarade avec une valeur test.';}
